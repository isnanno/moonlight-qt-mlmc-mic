...
void Session::execInternal()
{
    ...
    // Side-channel microphone passthrough (independent of Moonlight audio stream)
    MicrophonePassthroughManager micPassthrough(m_Preferences->parallelMicrophonePassthrough,
                                              m_Computer->activeAddress.address());
    if (micPassthrough.isActive()) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                    "Parallel microphone passthrough enabled to %s:%u",
